...
test('BRD-API-GET-001: Obtener un tablero existente @smoke @integration', async ({ request, makeBoard }) => {
...
test('BRD-API-GET-002: Obtener un tablero sin token @integration', async ({ request, makeBoard }) => {
  const board = await makeBoard();
  const res = await getBoard_noAuth(request, board.id);
  expectStatus(res, [400, 401, 403]);
});

test('BRD-API-GET-003: Obtener un tablero de un  boardId inexistente @integration', async ({ request }) => {
  const fakeId = '64f000000000000000000000';
  const res = await getBoard(request, fakeId);
  expectStatus(res, [400, 404]);
});

test('BRD-API-GET-004: Verificar que no muestre nada al equivocarse en un Endpoint incorrecto (sin {id}) @integration', async ({ request }) => {
  const res = await getBoards_collection(request);
  expectStatus(res, [400, 404, 405]);
});

test('BRD-API-GET-005: Verificar que arroje esquema de respuesta @integration ', async ({ request, makeBoard }) => {
  const board = await makeBoard();
  const res = await getBoard(request, board.id);
  expectStatus(res, 200);
  const body = await res.json();
  ...
  test('BRD-API-GET-006: Verificar que al ver un tablero tenga el Header Content-Type JSON @integration', async ({ request, makeBoard }) => {
  ...
  test('BRD-API-GET-007: Verificar que la visualización sea su Latencia < 2s @integration ', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const t0 = Date.now();
    const res = await getBoard(request, b.id);
    const t1 = Date.now();
  ...
  test('BRD-API-GET-008: Verificar que un tablero no tenga Coincidencia exacta de id y name @integration', async ({ request, makeBoard }) => {
  ...
  test('BRD-API-GET-009: Verificar que al Obtener un tablero tras actualizar  se visualice los cambios @integration', async ({ request, makeBoard }) => {
  ...
  test('BRD-API-GET-011: Verificar que al obtener un tablero se tenga los Campos mínimos presentes en schema @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const res = await getBoard(request, b.id);
    const body = await res.json();
    validateSchema(schema, body);
  ...
  test('BRD-API-GET-013: GET Verificar que se meustre un tablero al hacer dos veces clic en get repetido @integration @smoke', async ({ request, makeBoard }) => {
    const b = await makeBoard();
  ...
  test('BRD-API-GET-014: Verificar que no se muestre un tablero  tras DELETE debe fallar @integration', async ({ request, makeBoard }) => {
    const { deleteBoard } = require('../../../src/services/boards.service.js');
    const b = await makeBoard();
  ...
  test('BRD-API-GET-015: Verificar que no se visualice al ingresar un GET con id muy largo o inexistente @integration', async ({ request }) => {
    const res = await getBoard(request, 'x'.repeat(60));
    expectStatus(res, [400, 404]);
  });

  test('BRD-API-GET-016: Verificar qe al visualizar con id con espacios nos de status code de  400/404 @integration', async ({ request }) => {
    const res = await getBoard(request, '  bad id  ');
    expectStatus(res, [400, 404]);
  });


  test('BRD-API-GET-017: Validar tiempo estable en varias lecturas (<2s) @integration @smoke', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    for (let i = 0; i < 3; i++) {
      const t0 = Date.now();
      const res = await getBoard(request, b.id);
