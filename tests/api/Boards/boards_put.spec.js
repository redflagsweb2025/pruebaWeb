...
test('BRD-API-PUT-001: Actualizar un tablero el name y descripcion con datos válidos @integration @smoke', async ({ request, makeBoard }) => {
...
test('BRD-API-PUT-002: Actualizar un tablero sin sin token @integration', async ({ request, makeBoard }) => {
  const board = await makeBoard();
  const res = await updateBoard_noAuth(request, board.id, { name: 'NO_TOKEN' });
  expectStatus(res, [400, 401, 403]);
});

test('BRD-API-PUT-003: Actualizar un tablero con token inválido @integration', async ({ request, makeBoard }) => {
  const board = await makeBoard();
  const res = await updateBoard_invalidAuth(request, board.id, { name: 'BAD_TOKEN' });
  expectStatus(res, [401, 403]);
});

test('BRD-API-PUT-004: Actualizar un tablero  con parámetros inválidos (name vacío) @integration ', async ({ request, makeBoard }) => {
  const board = await makeBoard();
  const res = await updateBoard(request, board.id, { name: '' });
  expectStatus(res, [400, 401, 422]);
});

test('BRD-API-PUT-005: Actualizar un tablero sin permisos de administrador @integration ', async ({ request, makeBoard }) => {
  const board = await makeBoard();
  ...
  test('BRD-API-PUT-006: Verificar que tenga el Header Content-Type JSON @integration ', async ({ request, makeBoard }) => {
  ...
  test('BRD-API-PUT-007: Verificar que al actualizar un board su Latencia < 2s en update @integration ', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const t0 = Date.now();
  ...
  test('BRD-API-PUT-008: Verificar que permita  Actualizar solo descripcion @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
  ...
  test('BRD-API-PUT-009: Verificar que permita Actualizar name con caracteres de Unicode @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
  ...
  test('BRD-API-PUT-010: Verificar que permita Actualizar con emoji en name @integration @smoke', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const res = await updateBoard(request, b.id, { name: 'EMOJI 😀 ' + Date.now() });
    expectStatus(res, 200);
  });

  test('BRD-API-PUT-011: Verificar que no permita actualizar con el mismo nombre @integration', async ({ request, makeBoard }) => {
  ...
  test('BRD-API-PUT-012: Verificar que no acepte name largo <= 50 chars @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
  ...
  test('BRD-API-PUT-013: Verificar que no permita update sin cambios (parámetros vacíos) → 200/400/422 @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const res = await updateBoard(request, b.id, {}); // depende del API
    expectStatus(res, [200, 400, 422]);
  });

  test('BRD-API-PUT-014: Verificar que no permita actualizar con campos desconocidos o  ignorado @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const res = await updateBoard(request, b.id, { name: 'UNK', fooBar: 'x' });
    expectStatus(res, [200, 204]);
  });

  test('BRD-API-PUT-015: Verificar que elimine en name con espacios borde @integration ', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const res = await updateBoard(request, b.id, { name: '  TRIM_PUT  ' });
    expectStatus(res, [200, 204]);
  });

  test('BRD-API-PUT-016: Verificar que no permita actualizar en name con solo espacios → 400/422 @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const res = await updateBoard(request, b.id, { name: '   ' });
    expectStatus(res, [400, 401, 422]);
  });

  test('BRD-API-PUT-017: Actualizar después de DELETE → 400/404 @integration', async ({ request, makeBoard }) => {
  ...
  test('BRD-API-PUT-018: Validar headers y latencia en update @integration', async ({ request, makeBoard }) => {
    const b = await makeBoard();
    const t0 = Date.now();
